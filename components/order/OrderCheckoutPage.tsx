      ...
      <section className="border border-black/10 bg-white">
        <div className="border-b border-black/8 px-6 py-6">
          ...
          <p className="mt-3 max-w-2xl text-[15px] leading-7 text-black/72">
            주문자 정보와 입금자명을 입력하면 주문이 접수됩니다. 접수 완료 후
            아래 안내 계좌로 입금해 주세요.
          </p>
        </div>
            ...
            >
              ...
              {formState.status === "success" && formState.bankInfo ? (
                <div className="mt-3 space-y-1 text-[14px]">
                  {submittedOrderCodes.length > 1 ? (
                    <div className="space-y-1">
                      <p>주문번호</p>
                      {submittedOrderCodes.map((orderCode) => (
                        <p key={orderCode}>{orderCode}</p>
                      ))}
                    </div>
                  ) : (
                    <p>주문번호: {submittedOrderCodes[0]}</p>
                  )}
                  <p>입금금액: {formatPrice(formState.totalAmount ?? orderTotalPrice)}</p>

    ...
    <section className="px-5 pt-6 sm:px-8 sm:pt-8 lg:px-12">
      <div className="mx-auto grid max-w-[1440px] gap-4 lg:grid-cols-[minmax(0,0.98fr)_minmax(0,1.12fr)]">
        <div className="flex min-h-[520px] flex-col rounded-[28px] border border-black/10 bg-white/76 px-6 py-7 sm:px-10 sm:py-10 lg:px-12 lg:py-12">
          ...
          <p className="mt-7 max-w-xl text-[16px] leading-8 text-black/84 sm:mt-8 sm:text-[17px]">
            {content.description}
          </p>

          <div className="mt-9 flex flex-wrap gap-3">
            <a
              className="inline-flex h-12 items-center justify-center rounded-full bg-black px-6 text-[14px] font-medium text-white transition-colors duration-300 hover:bg-black/84"
              href="#new-in"
            >
              {content.primaryCta}
            </a>
            <a
              className="inline-flex h-12 items-center justify-center rounded-full border border-black/10 px-6 text-[14px] font-medium text-black transition-colors duration-300 hover:bg-black/4"
              href="#promo"
            >
          ...
          <div className="mt-auto grid gap-3 border-t border-black/10 pt-8 sm:grid-cols-3">
            {content.stats.map((stat) => (
              <div key={stat.label} className="space-y-1">
                <p className="text-[11px] font-medium uppercase tracking-[0.22em] text-black/56">
                  {stat.label}
                </p>
                <p className="text-[1.08rem] font-semibold tracking-[-0.04em] text-black">
                  {stat.value}
                </p>
              </div>
            ))}
        ...
        <div className="relative min-h-[440px] overflow-hidden rounded-[28px] border border-black/10 bg-[#101010] sm:min-h-[520px]">
          ...
          <div className="absolute left-6 top-6 text-[11px] font-medium uppercase tracking-[0.24em] text-white/62 sm:left-8 sm:top-8">
            urban mood / 2026
          </div>
          ...
          <div className="absolute bottom-5 left-5 w-[46%] rounded-[22px] border border-black/10 bg-[#e8e3db] p-5 text-black sm:bottom-6 sm:left-6 sm:p-6">
            <p className="text-[11px] font-medium uppercase tracking-[0.24em] text-black/62">
              Drop 01
            </p>
          ...
          <div className="absolute bottom-5 right-5 flex w-[40%] flex-col gap-4 rounded-[22px] border border-white/10 bg-white/5 p-4 backdrop-blur-sm sm:bottom-6 sm:right-6 sm:p-5">
            ...
            <div className="flex items-center justify-between text-[11px] font-medium uppercase tracking-[0.22em] text-white/72">
              <span>Quiet fit</span>
              <span>02</span>
            </div>
          </div>
